...
func TestMinusOperator(t *testing.T) {
	t.Run("int64(123)-int64(123)", func(t *testing.T) {
		...
		assert.True(t, ok2)
		assert.Equal(t, int64(0), int64Value.Int64Value())
	})

	t.Run("string(\"123\")-string(\"123\")", func(t *testing.T) {
		lexer := NewYaccIdlLexerImpl(
			0,
			[]lexem{
				{token: RWconst, data: nil},
	...
	t.Run("float64(123)-float64(123)", func(t *testing.T) {
		lexer := NewYaccIdlLexerImpl(
			0,
			[]lexem{
				{token: RWconst, data: nil},
		...
		assert.True(t, ok2)
		assert.Equal(t, float64(0), float64Value.FloatValue())
	})
}
